...
- Date: 2026-08-29
- Base: `AGENTS.md:purpose` core flow definition, `docs/design/specs/design-v2.md`, `schemas/flowspec.schema.json` v2, `docs/guides/llm-usage.md` v2, research 2026-08-29
...
Existing 7 tools keep their names and schemas (`docs/guides/llm-usage.md:58` table extended by one row).
...
- Concurrency: MCP `Server` is single-process; `storage.BeginGeneration` / `storage.Commit` is already atomic via pointer rename (`docs/design/specs/design-v2.md:6.3`). Concurrent `publish_core_flow` calls serialize on the generation commit — last writer wins for same flowId.
...
Unchanged: `.codeflow/` layout per `docs/design/specs/design-v2.md:6.1`. New artifact does not create a new top-level dir. Generation layout stays:
...
### 9.2 `docs/guides/llm-usage.md` delta
...
No CLI. Core flow publish is MCP-only (D8). The local CLI guide does not gain a `publish-core` CLI entry.
...
- Loopback + per-run token + Host/Origin check + CSRF defense stays per `docs/design/specs/design-v2.md:11.3`. `publish_core_flow` respects `checkAuth`.
...
- Phase 4 — SKILL.md v3 + `docs/guides/llm-usage.md` v3 (§1, §2, §3, §8). No CLI change (D8).
...
- Modify: `docs/guides/llm-usage.md` (§1, §2, §3, §8) — no CLI change per D8
