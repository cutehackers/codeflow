# CodeFlow 아키텍처 재설정 및 FlowSequence 전환 최종 명세서

- 문서 ID: `SPEC-CODEFLOW-ARCHITECTURAL-RESET-AND-FLOW-SEQUENCE`
- 상태: 승인됨 (Approved)
- 변경 정책: 구형 화면 모델의 타입, schema, endpoint, 저장 형식과의 호환성을 제공하지 않는 전면 변경
- 작성일자: 2026-09-16
- 문서 목적: 비본질적 부채(약 35,000 라인) 정리 목록 확정, 고정 7계층 아키텍처 강제 탈피, FlowSequence와 실행 타임라인의 비대칭 1:N 계층 관계 정립, 선택적 SLMLabeler 설계, CodeGraph 연동 지침 통합
...
2. **FlowSequence 전환**: 획일적 7계층 분류를 지양하고, 비즈니스 사건 중심의 핵심 관문(4~7개)으로 흐름을 요약.
3. **FlowSequence와 타임라인의 비대칭 1:N 계층 관계 정립**: 거시적 비즈니스 의미(FlowSequence)와 미시적 코드 실행 순서(Timeline)의 명확한 개념 분리 및 인터랙션 동기화.
4. **선택적 SLM 라벨러 아키텍처**: Go 코어 내 추론 엔진을 포함하지 않고, 로컬 모델 런타임(Ollama/vLLM)을 활용한 비동기 의미 라벨 제안 인터페이스를 제공합니다.
5. **CodeGraph와의 자연스러운 조화**: 전역 호출 그래프 색인(.codegraph)을 활용하여 수집·슬라이싱·레이더의 탐색을 보조하고, CodeFlow 고유의 독립성과 불변 스냅샷 검증을 엄격히 유지.
...
|---|---|---|---|
| **로컬 모델 런타임** | `internal/protocol/model_host*.go`<br>`internal/protocol/model_host_sandbox_darwin.go`<br>`internal/protocol/model_host_test.go` | 약 7,399줄 | **제거**: Go 코어 내에서 직접 로컬 SLM 바이너리를 fork/exec하고 OS Seatbelt 샌드박스를 구성하던 과도한 런타임. `SLMLabeler`가 로컬 모델 HTTP API를 사용하도록 대체. |
...
| **7계층 강제 추론기** | `internal/flowview/lanes.go`<br>(호출부: `server.go:1108` `applyLayersWith`) | 약 729줄 | **제거**: 임의의 프로젝트를 7계층에 억지로 끼워 맞추기 위한 729줄의 이웃 노드 투표/추론 알고리즘 및 `server.go` 호출부 제거. FlowSequence 큐레이션으로 대체. |
...
3. **`curator` (`internal/curator`)로 통합**:
   - `internal/semantic/flow_sequence.go`: 2단계 큐레이션 알고리즘(매크로 응집 + 중요도 선별) 기반 FlowSequence projection.
   - `SLMLabeler`: 선택적 로컬 SLM 연동을 통한 미시적 비즈니스 의미 라벨 제안 플러그인 신설.
...
5. **`agent-gateway` (`internal/agentgateway`)로 통합**:
   - `internal/mcp/`: 8대 핵심 표준 MCP 도구 stdio JSON-RPC 게이트웨이 및 고밀도 페이로드 직렬화기.

---

## 3. FlowSequence 관문과 실행 타임라인의 1:N 계층 관계
...
- 따라서 정적인 물리적 계층 분류를 최상위 분류 기준으로 삼지 않고, 실행 흐름에서 일어나는 **비즈니스 관문**을 중심으로 모델을 재구성합니다.

### 3.2 FlowSequence와 실행 타임라인의 명확한 정의
...
┌─────────────────────────────────────────────────────────────────────────────┐
│  [거시적 의미] FlowSequence                                                 │
│  - 단위: FlowSequence 관문 (FlowSequenceFrame)                              │
...
2. **2단계: 중요도 기반 선별 (Significance Scoring)**:
   ...
   - 상위 4~7개 핵심 관문만 FlowSequence 메인 카드로 승격하고, 하위 순위 관문은 인접 핵심 관문의 접힌 세부 목록으로 편입합니다.
...
3. **극단적 흐름 규모에 대한 경계 규칙 (Floor & Ceiling Bounds)**:
   ...
   - **상한선(Ceiling: 30개 파일, 150단계 이상의 대규모 분산 흐름)**:
     - 수십 개 파일에 걸친 150단계 이상의 방대한 트레이스라도, FlowSequence 메인 뷰에는 **최대 7개의 핵심 비즈니스 관문 카드만 노출**됩니다.
     ...
     - 큐레이션 알고리즘은 단일 패스 순회($O(N)$)로 구현하며, 처리 시간은 구현 후 측정해 성능 기준으로 확정합니다.

### 3.4 프론트엔드 명칭 정비 및 인터랙션 동기화
1. **헤더 텍스트 수정**:
   - `web/flowview/src/components/NavRail.svelte`의 `<h2>FlowSequence</h2>` 헤더를 사용합니다.
...
3. **로컬 구동 방식**:
   - Go 코어 내부에 무거운 C++ 추론 엔진이나 샌드박스를 심지 않고, 개발자 환경의 **로컬 런타임(Ollama / llama-server)과 표준 HTTP 규격(`POST http://localhost:11434/v1/chat/completions`)으로 통신하는 초경량 인터페이스**로 구현합니다.

### 4.2 CodeGraph + 로컬 SLM의 완벽한 로컬 3중주 시너지

```
┌─────────────────────────────────────────────────────────────────────────────┐
│ 1단계: CodeGraph (정적 뼈대 공급)                                             │
...
└──────────────────────────────────────┬──────────────────────────────────────┘
                                       ...
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│ 3단계: 메인 에이전트 (고차원 작업 집중)                                    │
│   - 수십 개 파일을 직접 읽지 않고, 의미 라벨이 추가된 관문 요약 수신          │
│   - 고차원 아키텍처 리팩토링 및 복잡한 버그 해결에 온전히 집중               │
└─────────────────────────────────────────────────────────────────────────────┘
```

- **근거 제한**: SLM에는 CodeGraph와 CodeFlow가 확인한 심볼·호출 사실만 제공하며, SLM 출력은 검증된 사실과 분리된 제안으로 취급합니다.
- **로컬 실행**: CodeGraph, CodeFlow, SLMLabeler는 기본적으로 인터넷 연결 없이 개발자 환경에서 실행할 수 있습니다. 지연 시간은 구현 후 측정해 별도 기준으로 관리합니다.

### 4.3 아키텍처 및 인터페이스 설계

```go
// FlowFrameLabel은 SLM이 특정 FlowSequence 관문에 제안한 비즈니스 의미 라벨입니다.
...
SLM 라벨 제안은 `POST /api/semantic/labels`로 요청합니다. 요청과 응답은 FlowSequence와 `FlowFrameLabel`을 기준으로 정의합니다.

### 4.4 비동기 의미 라벨 제안 파이프라인
1. **1단계 (정적 FlowSequence 렌더링)**:
   - 사용자가 흐름을 조회하면, Go 코어는 AST 정적 사실 기반의 FlowSequence를 먼저 반환합니다. 개발자는 SLM 응답을 기다리지 않고 코드 탐색을 시작합니다.
2. **2단계 (비동기 의미 라벨 제안)**:
   - 라벨러가 활성화된 경우, 백그라운드에서 로컬 모델 런타임을 호출하여 각 관문의 한 줄 한국어 비즈니스 라벨을 제안받습니다.
...
3. **무장애 폴백 및 비정상 응답 방어 (Fault Tolerance & Safe Fallback)**:
   ...
   - **타임아웃 보장**: 500ms(설정 가능) 경과 시 `context.WithTimeout`에 의해 HTTP 요청이 종료되며, 라벨 상태는 `timed_out`으로 표기됩니다.
...
### 5.1 역할의 명확한 분담
- **CodeGraph**: 리포지토리 전체의 함수, 클래스, 라우트 간 호출, 구현, 참조 관계를 질의하는 **전역 정적 지식 그래프 색인기** (`.codegraph/codegraph.db`).
- **CodeFlow**: 특정 사용자 요청에서 시작하여 검증된 실행 경로를 추출하고, 비즈니스 관문 FlowSequence로 시각화하는 **비즈니스 흐름 이해 워크벤치**.
...
[사용자 질의: "결제 흐름 분석해줘"]
 ...
 │    - CodeGraph 색인 질의로 진입점 탐색 비용을 줄임                │
 └────────────────────────────┬────────────────────────────────┘
                              ▼
 ┌─────────────────────────────────────────────────────────────┐
 ...
 │    - CodeGraph `callers -j` / `impact -j`로 관계 산출            │
 └────────────────────────────┬────────────────────────────────┘
                              ▼
 ┌─────────────────────────────────────────────────────────────┐
 │ 4. AI 에이전트 컨텍스트 축소 (Compact Agent Context)        │
 │    - 파일 수십 개를 직접 읽는 중복 탐색 감소                 │
 │    - CodeGraph 소스 + CodeFlow 3열 뷰모델 조합              │
 └─────────────────────────────────────────────────────────────┘
```
...
3. **영향도 레이더(Blast Radius Radar) 즉시 렌더링**:
   - 우측 레이더 렌더링 시 CodeGraph의 호출자·피호출자 색인을 조회하여 1단계 직접 관계망을 구성합니다.
4. **AI 에이전트 컨텍스트 축소**:
   - AI 에이전트가 여러 파일을 중복해서 열어보는 방식을, **CodeGraph 소스 탐색 + CodeFlow의 정제된 FlowSequence 페이로드(`get_flow_payload`)** 조합으로 줄입니다.

### 5.3 결합 분리 안전 불변식 및 소스 불일치 회복력
- **독립성 보장 (Zero Hard Dependency)**: `.codegraph/`가 없거나 데몬이 정지되어 있어도 CodeFlow는 자체 내장 AST 어댑터로 동작합니다.
...
flowchart TD
    ...
    subgraph M3 ["3. 스토리 큐레이터 모듈: curator"]
        ...
        SC3["SLMLabeler [선택적]<br/>로컬 SLM 기반 미시적 비즈니스 의미 라벨 제안"]
    end
    ...
    subgraph M5 ["5. 에이전트 전용 관문: agent-gateway (Go 패키지: agentgateway)"]
        ...
        AG2["CompactPayloadSerializer<br/>고밀도 페이로드 생성"]
    end

    M1 ==>|정적 뼈대 & AST 사실| M2
    M2 ==>|검증된 실행 스텝 목록| M3
    M3 ==>|정제된 5~7개 FlowSequence| M4
    M3 ==>|정제된 5~7개 FlowSequence| M5
```
...
| **`curator`**<br>(`internal/curator`) | *"수백 개 스텝 중 핵심 비즈니스 관문은 무엇인가?"* | • 매크로 블록 응집 (`MacroClumper`): 연속 if/상태변경 병합<br>• 중요도 선별 (`SignificanceRanker`): 4~7개 핵심 관문 압축<br>• 의미 라벨 제안 (`SLMLabeler`): 로컬 SLM 기반 1줄 라벨 생성 |
...
| **`agent-gateway`**<br>(`internal/agentgateway`) | *"AI 에이전트가 흐름을 즉시 이해하는가?"* | • 표준 8대 MCP 도구 stdio JSON-RPC 제공 (`MCPServer`)<br>• 고밀도 FlowSequence 페이로드 공급<br>• **AI 에이전트 전용 접근 관문** |
...
3. **독립적 단위 테스트 가능**: 웹 서버나 DB 없이도 `curator`의 2단계 큐레이션 알고리즘을 독립 단위 테스트로 검증할 수 있습니다.

### 6.2.1 전 slice 공통 불변식

- `INV-PARENT-01`: 모든 source 위치, 관계, frame은 동일한 snapshot identity와 Evidence 상태를 유지합니다.
- `INV-PARENT-02`: 검증되지 않은 사실과 static relation은 `verified` 또는 runtime observation으로 승격하지 않습니다.
- `INV-PARENT-03`: SLMLabeler의 출력은 의미 라벨 제안으로만 취급하며 Fact, Evidence, source anchor를 변경하지 않습니다.
- `INV-PARENT-04`: CodeGraph는 선택적 가속 수단이며, 미설치·오류·stale 상태에서도 AST fallback으로 동일한 FlowSequence 계약을 제공합니다.
- `INV-PARENT-05`: 이번 전면 변경에서 구형 화면 모델의 타입, schema, endpoint, 저장 형식은 읽거나 자동 변환하지 않습니다.
- `INV-PARENT-06`: 내부 compiler·index·scheduler telemetry와 비밀값은 primary view 또는 외부 응답에 노출하지 않습니다.
...
2. **`collector` (흐름 수집·슬라이서 - 단독 동작)**:
   - **단독 기능**: 특정 진입점 심볼이 주어지면, 화면 표출이나 FlowSequence 가공 없이도 해당 심볼에서 시작하는 원시 실행 경로(AST 호출 순서, 가드 조건, 상태 변경)를 순수하게 슬라이싱하여 추출합니다.
...
3. **`curator` (스토리 큐레이터/의미 부여기 - 단독 동작)**:
   - **단독 기능**: 원시 실행 트레이스 JSON(`RawExecutionTrace`)만 주어지면, 소스코드 파서나 네트워크 연결 없이도 순수 인메모리 함수형 알고리즘으로 4~7개 매크로 관문으로 압축한 `FlowSequence`를 생성합니다.
   - **입력**: `RawExecutionTrace`
   - **출력**: `FlowSequence` (4~7개 frame + 1:N 실행 상세 + 선택적 의미 라벨)
   - **단독 CLI/테스트**: `codeflow curate <trace.json>` / `curator_test.go`
4. **`presenter` (플로우뷰 워크벤치 - 단독 동작)**:
   - **단독 기능**: 정제된 `FlowSequence` JSON만 주어지면, 앞단의 정적 파서나 슬라이서가 없어도 독립적으로 3열 워크벤치 웹 UI(FlowSequence, 코드 렌즈, 레이더)를 띄워 개발자가 탐색할 수 있도록 서빙합니다.
   - **입력**: `FlowSequence` 데이터 모델 (저장된 JSON 파일 포함)
   - **출력**: 로컬 HTTP 서버 및 Svelte 5 3열 인터랙티브 웹 UI
   - **단독 CLI/테스트**: `codeflow view <flow-sequence.json>` / `presenter_test.go`
5. **`agent-gateway` (에이전트 게이트웨이 - 단독 동작)**:
   - **단독 기능**: 웹 브라우저나 UI 없이도, AI 에이전트의 stdio JSON-RPC 요청을 받아 독립적으로 처리하고 고밀도 압축 페이로드를 반환하는 헤드리스(Headless) 게이트웨이로 단독 구동됩니다.
...
### 7.1 3열 레이아웃 보존
기존 Svelte 5 워크벤치([`web/flowview/src/App.svelte`](web/flowview/src/App.svelte)) 구조는 단 1픽셀도 버리지 않고 그대로 유지합니다:
- **상단**: FlowView 상태 및 질의 영역.
- **좌측**: FlowSequence (관문 카드 목록 및 내부 타임라인 스텝 아코디언).
- **중앙**: 메인 코드 렌즈 뷰포트 (선택된 코드 라인 및 주변 컨텍스트).
- **우측**: 맥락 및 영향도 레이더 (1단계 직접 호출자/피호출자 관계망).

### 7.2 7대 화면 결함 해결 명세 ([`2026-09-15-flowview-review-corrections-ko.md`](docs/design/specs/2026-09-15-flowview-review-corrections-ko.md))
1. **후보 ID vs 흐름 ID 불일치 해결**: 발원지인 [`internal/flowview/server.go:1880`](internal/flowview/server.go#L1880)에서 `FlowID: resolved.CandidateID`를 `resolved.FlowID`로 정상 치환합니다.
2. **FlowSequence URL 소스 문맥 복원 실패 해결 (1순위 긴급 결함)**: [`internal/flowview/task_view_persistence.go`](internal/flowview/task_view_persistence.go)의 `RestoreFlowSequenceContext`에서 `flowContexts`와 `sourceFiles`를 복원하여, 저장 뷰 로딩 시 *"이 분석에 연결된 소스가 없습니다"*가 표시되지 않게 합니다.
3. **거짓 검증 상태 수정**: [`internal/semantic/flow_sequence.go:196-199`](internal/semantic/flow_sequence.go#L196-L199)에서 근거 코드의 `ValidationStatus`가 부재하거나 `unknown`인 frame에 대해 `verified` 표기를 엄격히 금지합니다.
4. **컴파일러 경계 frame 매칭**: [`internal/semantic/compiler.go:211`](internal/semantic/compiler.go#L211) 및 [`flow_sequence.go:232-235`](internal/semantic/flow_sequence.go#L232-L235)의 식별자 합성 키를 정규화하여 분석 경계가 FlowSequence frame에 정상 노출되도록 수정합니다.
5. **레이더 마우스 호버 좌표 이탈 해결**: [`web/flowview/src/components/BlastRadiusRadar.svelte:323-326`](web/flowview/src/components/BlastRadiusRadar.svelte#L323-L326)의 SVG 좌표와 충돌하는 CSS `scale(1.04)`를 제거하여 호버 시 노드가 (0,0)으로 튀는 현상을 해결합니다.
6. **조건 필터 잠김(Lockup) 해결**: [`web/flowview/src/App.svelte:109`](web/flowview/src/App.svelte#L109)에서 분기문이 없는 장면에 진입해도 조건 선택창이 영구 비활성화되지 않도록 필터 해제 옵션을 상시 노출합니다.
...
1. **슬라이싱 수집 단계(`collector`)**:
   ...
   - 동일 심볼 경로로의 재진입(순환 또는 자기 재귀)이 감지되면 [`internal/semantic/models.go:28`](internal/semantic/models.go#L28)의 `SemanticEdge` 중 `Kind == "cycle"` 또는 `ResolutionStatus == "cycle"`인 역방향 간선(Back-edge)을 명시적으로 기록하고, 탐색 깊이를 최대 1회 반복(Max Unrolling Depth = 1)으로 제한하여 슬라이싱을 즉시 중단합니다. 이를 통해 무한 AST 탐색과 메모리 고갈을 원천 차단합니다.
2. **FlowSequence 큐레이터 단계(`curator`)**:
...
2. **불변 스냅샷 기반 자기 검증 (Self-Validation)**:
   - CodeGraph가 반환한 심볼 위치(`byteRange`, `line`)에 대해, [`internal/fusion/fusion.go:380-430`](internal/fusion/fusion.go#L380-L430)의 `checkFreshnessBytes` 및 `SpanHash` 검증 파이프라인을 통과시켜 최신 불변 파일 스냅샷 SHA-256 해시 및 바이트 텍스트를 즉각 검증합니다.
3. **2단계 회복 폴백 (Fallback Strategy)**:
   - **1단계 (앵커 재조정, Re-anchoring)**: 바이트 내용이 심볼 시그니처와 불일치할 경우, `fusion.go`의 선언문 정규식/AST 재탐색을 통해 최신 라인 및 바이트 범위를 다시 계산합니다.
...
2. **3대 동기화 방어 메커니즘**:
   - **1단계: Git HEAD 정합성 선제 핸드셰이크 (Head Revision Handshake)**:
     - `CodeGraphClient`는 질의 시 현재 작업 공간의 `.git/HEAD` 커밋 해시와 CodeGraph 메타데이터의 색인 커밋 해시를 대조합니다.
   - **2단계: 불일치/재색인 락 감지 시 'CodeFlow 자체 AST 슬라이서'로 즉시 무중단 바이패스 (Automatic AST Bypass)**:
     ...
     - 현재 체크아웃된 작업 트리의 실제 디스크 소스 파일을 직접 파싱하여 최신 작업 트리 기준으로 실행 흐름을 분석합니다.
...
1. **비정상 응답 포맷 방어 (Malformed JSON)**:
   ...
   - `SLMLabeler`는 정규식 기반 JSON 추출기(`ExtractJSONPayload`)를 통해 순수 JSON 블록만 선제 추출하여 파싱합니다.
   - JSON 구조가 여전히 깨져 있거나 스키마와 불일치할 경우, 에러 모달 팝업 없이 조용히 AST 기본 사실 텍스트를 유지(Silent Fallback)합니다.
2. **엄격한 타임아웃 단절**:
   - 500ms 경과 시 `context.WithTimeout`에 의해 로컬 모델 HTTP 요청이 종료되며, 라벨 상태를 `timed_out`으로 기록합니다.
3. **화면 전환 시 지연 응답 폐기 (In-Flight Request Cancellation & Discarding)**:
   - 사용자가 SLM 비동기 호출 도중 다른 FlowSequence frame을 클릭하거나 새로운 흐름을 요청한 경우:
...
2. **대규모 분산 흐름 (30개 파일, 150단계 이상)**:
   - 30개 파일에 걸친 150단계 이상의 대규모 트레이스라도, FlowSequence 메인 뷰에는 **최대 7개의 핵심 비즈니스 관문 카드만 노출**됩니다.
   - 나머지 단계는 관문 하위 아코디언(`stepRefs`)에 보존되어 필요할 때 세부 라인을 펼쳐 확인할 수 있습니다.
   - 큐레이션 알고리즘은 단일 패스 순회($O(N)$)로 작동하며 처리 시간은 구현 후 측정합니다.

### 8.5 진입점 미발견(Zero Entrypoints) 시 안내 및 회복 워크플로우
1. **발생 상황 및 에러 규격**:
   - 리포지토리가 지원 프레임워크의 표준 라우트 구조를 갖추지 않았거나, 소스 디렉터리가 비어 있어 `EntryHarvester`가 진입점 후보를 0개 발견한 경우.
   - [`internal/semantic/query.go:219`](internal/semantic/query.go#L219)에서 후보 목록이 비어 있을 때 500 장애 대신 명시적인 `code: "no_entrypoints_found"` (400 Bad Request)를 반환하도록 분기를 정비합니다.
2. **무중단 회복 안내 워크플로우**:
...
```
[1단계: UI 명칭 정비 및 결함·회복력 해결] ➔ [2단계: 비본질적 부채 35k 라인 제거] ➔ [3단계: 5대 모듈 재배치 & FlowSequence 큐레이터/CodeGraph] ➔ [4단계: SLMLabeler]
```
...
### 3단계: 5대 모듈 재배치 및 FlowSequence 큐레이터/CodeGraph 연동
...
### 4단계: 선택적 SLMLabeler 구축
- `curator` 내 `SLMLabeler` 구현 및 `codeflow.config.json` 연동.
- 비정상 마크다운/JSON 전처리 추출기, 500ms 타임아웃, 화면 이탈 시 지연 응답 폐기 로직 완성.
- CodeGraph가 확인한 사실을 입력으로 사용하는 SLM 의미 라벨 제안 파이프라인 완성.

---

## 10. 완료 및 검증 기준 (Acceptance Criteria)

- [ ] **AC-01 (클린 아키텍처)**: 제거 대상 코드가 정리되고, 5대 모듈 단방향 의존성을 준수하며, `make fmt`, `make vet`, `CGO_ENABLED=0 make test`가 통과한다.
- [ ] **AC-02 (FlowSequence 큐레이션)**: 임의의 대형 실행 흐름에 대해 FlowSequence에 최대 7개의 핵심 비즈니스 관문 카드만 노출된다.
- [ ] **AC-03 (1:N 계층 인터랙션)**: 프레임 클릭 시 관문 대표 코드로 이동하고, 아코디언 내부 스텝 클릭 시 해당 코드 라인이 정밀 하이라이트된다.
- [ ] **AC-04 (CodeGraph 조화)**: `.codegraph/`가 있는 환경에서는 CodeGraph를 사용해 진입점과 직접 관계를 탐색하고, 미설치 환경에서는 자체 AST 분석으로 동작한다.
- [ ] **AC-05 (선택적 SLM 라벨링)**: SLM 라벨러가 활성화되면 정적 FlowSequence를 먼저 표시하고, 이후 의미 라벨을 비동기로 제안한다.
- [ ] **AC-06 (FlowView UX 무결성)**: 3열 레이아웃을 유지하며 레이더 노드 튐, 조건 필터 잠김, 소스 미표시 결함이 발생하지 않는다.
...
- [ ] **AC-08 (CodeGraph 불일치/브랜치 전환 자기 검증 및 SLM 폴백)**: Git 브랜치 전환이나 색인 불일치가 감지되면 CodeFlow 자체 AST 슬라이서로 전환하고, 앵커 불일치 시 스냅샷 기준으로 위치를 다시 계산하며, SLM 지연·파싱 오류 시 기본 정적 텍스트를 유지하고 화면 이탈 시 지연 응답을 폐기한다.
