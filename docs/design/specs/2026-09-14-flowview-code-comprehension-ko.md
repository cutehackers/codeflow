# FlowView FlowSequence 코드 이해 최종 스펙
...
- Source: 기존 FlowView 코드 이해 스펙, 현재 구현 조사, 사용자 지시. FlowSequence를 기본 화면으로 사용하고, 현재 디자인을 보존하며, Live Semantic Map을 제거한다.
- Decision Records: [FlowView 개선 방향](../decisions/2026-09-14-flowview-code-comprehension-ko.md), [FlowView 스토리보드 전환 결정](../decisions/2026-09-15-flowview-storyboard-transition-ko.md)
...
FlowView는 사용자가 이해하려는 코드 동작을 FlowSequence 관문으로 보여준다. 각 관문은 시작, 판단, 처리, 외부 효과, 결과 또는 분석 경계처럼 이해에 필요한 사건을 나타낸다. 프로젝트별 architecture는 관문을 나누는 기준이 아니라 선택한 관문의 보조 정보다.

| Intent | Goal | 관찰 가능한 결과 |
|---|---|---|
| INT-FLOW-01 | GOAL-FLOW-01 | 사용자는 FlowSequence에서 코드 동작의 시작부터 결과 또는 분석 경계까지 이해한다. |
| INT-FLOW-01 | GOAL-FLOW-02 | 사용자는 선택한 관문의 실제 source, 조건, 직접 연결을 확인한다. |
| INT-FLOW-02 | GOAL-FLOW-03 | 사용자는 명시적으로 다시 분석한 결과 또는 선택한 두 분석의 차이를 확인한다. |
| INT-FLOW-02 | GOAL-FLOW-04 | 사용자는 선택한 관문의 검증된 직접 연결을 따라 다른 FlowSequence를 보고 원래 위치로 돌아온다. |

## 2. 범위

### 포함 범위

- FlowSequence projection, source context, partial 결과와 분석 경계 표시
- 현재 Svelte FlowView의 FlowSequence, 코드 뷰, 좌측 rail, 조건 강조, Blast Radius Radar, 3열 레이아웃과 시각 언어 보존
...
- source 근거 없이 관문 제목, 처리 연결, 실행 결과를 만들어 내는 동작
...
4. 현재 디자인에는 FlowSequence, 코드 뷰, 좌측 rail, source context, Radar가 이미 있다. 이 컴포넌트는 유지 대상이다.
...
FlowSequence는 source와 검증된 연결에서 만든 순서 있는 관문과 분기 집합이다. 관문은 단일 함수, 클래스, 파일 또는 architecture label과 일대일 대응하지 않는다.
...
위 조건을 만들지 않는 연속 내부 step은 같은 관문에 묶는다. architecture label, 패키지 경계, 파일 변경, 함수 호출 수는 단독으로 관문 분리 근거가 아니다.

### 4.2 `FlowSequenceProjection`

`FlowSequenceProjection`은 canonical semantic map에서 파생하는 화면 전용 결과다. 기존 map, Evidence, public payload를 바꾸지 않는다. public 결과로 노출해야 하면 별도 schema version과 producer·consumer fixture를 먼저 추가한다.

| 필드 | 규칙 |
|---|---|
| `frameId` | 같은 분석 안에서 안정적으로 선택할 수 있는 관문 식별자 |
...
| `stepRefs` | 관문을 뒷받침하는 canonical step 목록 |
...
- 기본 화면은 `FlowSequenceProjection`의 관문과 관문 사이의 확인된 관계만 렌더링한다.
- `semanticMap.steps`, 모든 edge, 모든 source context는 기본 화면의 목록으로 렌더링하지 않는다.
- 관문을 선택하면 해당 관문의 primary source context를 연다. 접힌 detail과 직접 relation은 사용자가 요청했을 때만 연다.
...
`FlowSequenceView`, `NavRail`, `CodeFlowPanel`, `ProcessFlowPanel`, `ContextAside`, Blast Radius Radar, 선택 카드, 코드 강조, 반응형 3열 레이아웃의 시각적 구조와 상호작용을 보존한다.

변경은 컴포넌트의 위치나 디자인 언어가 아니라 데이터의 우선순위다.

| 표면 | 유지할 동작 | 변경할 동작 |
|---|---|---|
| FlowSequence | 카드, track, 선택 스타일, source로 이동 | 전체 step 대신 FlowSequence 관문 표시 |
| 좌측 rail | 관문 선택과 현재 선택 강조 | step 목록 대신 관문 목록 표시 |
| 코드 뷰 | 코드/처리 보기, 조건 강조, 접기·펼치기 | 선택 관문의 source를 먼저 표시 |
...
| 사용자 동작 | 결과 |
|---|---|
| FlowSequence 보기 | entry 후보를 해석하고 하나로 확인된 snapshot을 분석해 FlowSequence를 표시한다. |
...
| 직접 연결 보기 | 선택 관문의 같은 snapshot 직접 relation만 연다. |

edit event, watcher event, SSE event, background publication은 위 동작 어느 것도 시작하거나 화면을 교체하지 않는다.

## 6. Live Semantic Map 제거와 호환성

제거는 FlowView 구현과 검증이 통과한 뒤에 수행한다. 먼저 FlowSequence 화면을 완성한 뒤 Live 경로를 삭제한다. 이 순서는 사용자 경험의 공백을 막기 위한 것이다.
...
1. FlowSequence의 모든 source와 relation은 같은 immutable snapshot의 근거를 사용한다.
2. 화면은 명시적 사용자 요청에 성공한 결과만 교체한다. 늦은 응답, 취소, 실패는 현재 선택, source 위치, 펼침 상태, baseline을 바꾸지 않는다.
3. 선택한 관문이 새 분석에서 대응되지 않으면 임의 관문으로 이동하지 않고 기존 위치와 이유를 보존한다.
...
| ID | 질문 | 결정 | 기각한 대안 | 결과 |
|---|---|---|---|---|
| 결정1 | 기본 화면의 조직 기준은 무엇인가 | FlowSequence 관문 | architecture lane, 전체 step timeline | 서로 다른 프로젝트에도 같은 이해 단위를 제공한다. |
| 결정2 | architecture 정보는 어떻게 쓰는가 | 선택 관문의 보조 label | 모든 프로젝트에 고정 계층 적용 | architecture를 확인하지 못해도 화면이 동작한다. |
| 결정3 | 현재 디자인은 어떻게 하는가 | FlowSequence·코드 뷰·Radar·3열 레이아웃 유지 | 화면 전체 재설계 | 구현 범위를 정보 구조와 Live 제거로 제한한다. |
...
- FA-01: 사용자가 흐름을 조회하면 시스템은 FlowSequence 관문, 시작, 결과 또는 분석 경계와 해당 source를 표시한다.
- FA-02: 시스템은 architecture label이 없거나 프로젝트별 구조가 달라도 FlowSequence를 구성한다.
- FA-03: 시스템은 내부 step을 기본 화면에 모두 나열하지 않고, 관문 분리 규칙에 맞지 않는 detail을 접는다.
- FA-04: 사용자가 관문을 선택하면 FlowSequence, rail, 코드 뷰, Radar가 같은 primary step과 source 위치를 선택한다.
...
- FA-14: 실제 코드 이해 과제에서 FlowSequence가 기존 전체-step 화면보다 이해 시간, 정답률, 재탐색, 잘못된 확신에 주는 영향을 기록한다. 수치와 합격 기준은 평가 전에 고정한다.

## 10. 검증 계획

| 검증 | 필수 증거 |
|---|---|
| projection | 관문 분리·묶기, branch/failure/effect/boundary 보존, architecture 부재, 접힌 detail을 Go 단위·contract test로 검증 |
| task-view HTTP | projection과 map/Evidence/source identity 일치, 기본 화면에 전달할 관문 선택, partial·unknown·오류 응답 검증 |
| Svelte store·component | raw step 대신 FlowSequence 관문을 렌더링하고 선택 상태가 모든 기존 표면에 동기화됨을 검증 |
| browser | query, 관문 선택, keyboard, 긴 source, 실패, partial, 재분석, 비교, relation 왕복, 반응형 레이아웃을 실제 제품 화면에서 검증 |
...
1. canonical map에서 `FlowSequenceProjection`을 만들고, current endpoint와 Svelte store가 이를 기본 데이터로 사용하게 한다.
2. 현재 디자인 안에서 FlowSequence, rail, 코드 뷰, Radar 선택 동기화를 유지한 채 raw-step 기본 렌더링을 제거한다.
...
차단하는 제품 미결정은 없다. 관문 제목을 위한 model enrichment(SLMLabeler)는 선택 기능이며, 근거 없는 제목이나 연결을 만들 수 없다. 지원 언어별 실제 corpus와 사용자 평가 rubric은 구현 전에 고정한다.
