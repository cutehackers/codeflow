...
2. **Execution Phase (On-Demand & Scoped)**:
   ...
   * Executes [`detect.Detect`](../../../internal/detect/detect.go) on the target repository.
...
### 3.1 Server Bootstrap & Multi-Language Registry ([`internal/mcp/server.go`](../../../internal/mcp/server.go))
...
## 4. One-Shot Installer Specification ([`scripts/install.sh`](../../../scripts/install.sh))
...
## 5. Adapter Engine Hardening ([`internal/harvest/run.go`](../../../internal/harvest/run.go))
