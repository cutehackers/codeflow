...
- **Version Management & Release Synchronization**: When preparing a release or bumping a version (`vX.Y.Z`), the agent MUST follow `docs/VERSIONING.md` and synchronize all version locations simultaneously (`README.md`, `scripts/install.sh`, `internal/pin/compatibility.json`, and adapter package manifests) before suggesting release tags.
