...
> **대규모 코드베이스에서 비즈니스 핵심 흐름(Core Flow)을 엔드투엔드로 추출하고 시각화하는 개발자 및 AI 에이전트 도구**

CodeFlow는 진입 계층(UI 이벤트, API 요청 등)부터 컨트롤러, 유스케이스, 도메인, 데이터/외부 연동 계층까지의 **아키텍처 레이어 관통 경로(Core Flow)**를 검증된 코드 근거(Anchor)와 함께 추출하고, 인터랙티브한 **FlowView** 레인 맵으로 시각화합니다.
...
AI 에이전트(Codex, Claude, Antigravity 등)에게 [`docs/llm-usage.md`](docs/llm-usage.md)를 전달하거나 프롬프트에 참조하도록 지정하면, **사용자의 별도 설명 없이도** 에이전트가 다음을 자율적으로 수행합니다:

1. `bash scripts/install.sh`를 실행하여 CodeFlow 도구 및 MCP 환경 구축
...
Codex 또는 AI 에이전트에게 다음과 같이 자연어로 요청하세요:
...
"결제 실패 시의 예외 처리 및 롤백 브랜치 흐름을 보여줘"
```
...
- **Core Flow v3 아키텍처 사양**: [`docs/specs/core-flow-v3-spec.md`](docs/specs/core-flow-v3-spec.md)
- **전체 설계 문서**: [`docs/design-v2.md`](docs/design-v2.md)
