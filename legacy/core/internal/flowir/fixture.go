...
func Fixture(repo string, basis Basis) (Document, error) {
	...
	action := Fact{ID: Hash(SchemaVersion, "user_action", "signup-button", "submit", "fixture-action"), Kind: "user_action", Subject: "signup-button", Object: "submit", Evidence: []Anchor{anchor}, Status: Observed}
	...
	document := Document{SchemaVersion: SchemaVersion, Basis: basis, Facts: []Fact{result, entry, action}, Current: Flow{ID: "route:/signup", FlowKey: "route:/signup", EntryPointFact: entry.ID, Steps: []Step{step1, step2}, Status: Observed}}
	DeriveScenarios(&document)
	return document, nil
}
