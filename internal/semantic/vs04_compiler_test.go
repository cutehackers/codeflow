...
func TestVS04CompilerRequiresSnapshotAndUsesStructuralEvidence(t *testing.T) {
	...
	validatedSnapshot, err := rflscvs02.SnapshotInputFromContent("snapshot-demo", "basis-demo", "tree-demo", "", "deps-demo", 4, map[string]string{"service.go": string(content)})
	if err != nil {
		t.Fatal(err)
	}
	...
	shiftedInput := func() *rflscvs02.SnapshotInput {
		s, _ := rflscvs02.SnapshotInputFromContent("snapshot-demo-2", "basis-demo-2", "tree-demo-2", "", "deps-demo-2", 5, map[string]string{"service.go": string(shifted)})
		return &s
	}()
...
func TestVS04CompilerRequiresValidatedVS02SliceMetadata(t *testing.T) {
	...
	sliced := &slicing.SlicedPayload{CandidateID: target.CandidateID, EntrySymbolPath: target.EntrySymbolPath, Steps: []slicing.SliceStep{step}}
	input, err := rflscvs02.SnapshotInputFromContent("snapshot-metadata", "basis-metadata", "tree-metadata", "", "deps-metadata", 7, map[string]string{"service.go": string(content)})
	if err != nil {
		t.Fatal(err)
	}
	opts := CompileOptions{
...
func TestVS04CompilerRejectsUnconfirmedIntent(t *testing.T) {
	...
	input, err := rflscvs02.SnapshotInputFromContent("snapshot-confirm", "basis-confirm", "tree-confirm", "", "deps-confirm", 1, map[string]string{"service.go": string(content)})
	if err != nil {
		t.Fatal(err)
	}
