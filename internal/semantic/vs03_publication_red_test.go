...
func TestVS03PublicationGateRejectsIdentityDriftAndReportsMeasuredGap(t *testing.T) {
	...
	mapIR := vs03TestMap("basis-a", "snap-a", "task-a", 4)
	closure := vs03TestClosure("basis-a", "read-a", "closure-a", "task-a", 4)
...
func TestVS03PublicationGate_AllStrictIdentityAndObservationGatesPass(t *testing.T) {
	gate := NewPublicationGate()
	mapIR := vs03TestMap("basis-a", "snap-a", "task-a", 2)
	closure := vs03TestClosure("basis-a", "read-a", "closure-a", "task-a", 2)
	...
	canonical := rflscvs02.Result{
		SchemaID: rflscvs02.AnalyzerResultSchemaID, SchemaVersion: rflscvs02.SchemaVersion, RequestID: "request-a", Operation: "detect",
		...
		ReadSet:    rflscvs02.AnalysisReadSet{SchemaID: rflscvs02.ReadSetSchemaID, SchemaVersion: rflscvs02.SchemaVersion, ReadSetID: "read-a", ComputedBasisID: "basis-a", WorkspaceEpoch: 2, Documents: []rflscvs02.ReadDocument{}, NegativeObservations: []rflscvs02.Observation{}, MembershipObservations: []rflscvs02.Observation{{Kind: "membership", Path: ".", ValueHash: "membership-a", Measured: true}}, DependencyFrontiers: []rflscvs02.Observation{}},
		Closure:    rflscvs02.ObservationClosure{SchemaID: rflscvs02.ClosureSchemaID, SchemaVersion: rflscvs02.SchemaVersion, ClosureID: "closure-a", AnalysisReadSetID: "read-a", ComputedBasisID: "basis-a", WorkspaceEpoch: 2, Status: "closed", RequiredObservations: []string{"membership"}, MeasuredObservations: []string{"membership"}, NegativeObservations: []rflscvs02.Observation{}, MembershipObservations: []rflscvs02.Observation{{Kind: "membership", Path: ".", ValueHash: "membership-a", Measured: true}}, DependencyFrontiers: []rflscvs02.Observation{}, ClosureDigest: "closure-digest-a"},
		Capability: rflscvs02.CapabilityProfile{Adapter: "typescript", AnalyzerRevision: "analyzer/1", Features: []string{"snapshot_bytes"}},
		Coverage:   rflscvs02.Coverage{IncludedSourceRoots: []string{"."}, Measured: true}, Payload: []byte(`{"language":"typescript","confident":true}`), Diagnostics: []rflscvs02.Diagnostic{},
	}
	request, err := rflscvs02.NewAnalyzerRequest(canonical.RequestID, canonical.Operation, rflscvs02.SnapshotInput{SnapshotID: "snap-a", ComputedBasisID: "basis-a", RootTreeID: "tree-a", DependencyFingerprint: "dep-a", ConfigurationFingerprint: "cfg-a", WorkspaceEpoch: 2, Documents: []rflscvs02.SnapshotDocument{}}, nil, []string{"membership"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestVS03PublicationGateRejectsOpenClosureAndChangedMembership(t *testing.T) {
	gate := NewPublicationGate()
	mapIR := vs03TestMap("basis-a", "snap-a", "task-a", 1)
	closure := vs03TestClosure("basis-a", "read-a", "closure-a", "task-a", 1)
...
func vs03TestMap(basis, snapshot, task string, rev int) *SemanticMapIR {
...
func vs03TestClosure(basis, readSet, closureID, task string, rev int) *CausalObservationClosure {
