// Package isolation contains the executable acceptance runners for the
// evidence-bounded failure contract. The package is separate from
// the isolation package because the production one-shot executor imports the runtime
...
const (
	implementationPackage = "codeflow/internal/verification/isolation"
	basisID               = "basis-vs06"
...
func RunA07(t *testing.T) Evidence {
	...
	command := oneshoot.RuntimeCommand{Command: "configured-analyzer", Args: []string{"--snapshot"}}
...
func RunA09(t *testing.T) Evidence {
	...
	baseSpec := runtimeSpec(oneshoot.RuntimeCommand{Command: bin})
	...
	terminalCases := []struct {
		...
		{name: "success", want: oneshoot.RuntimeTerminalSuccess, ctx: func() (context.Context, context.CancelFunc) { return context.Background(), func() {} }},
		{name: "failure", env: []string{"MOCK_CRASH_AFTER_N_REQUESTS=1"}, want: oneshoot.RuntimeTerminalFailure, ctx: func() (context.Context, context.CancelFunc) { return context.Background(), func() {} }},
		{name: "timeout", env: []string{"MOCK_HANG_OPS=detect"}, want: oneshoot.RuntimeTerminalTimeout, ctx: func() (context.Context, context.CancelFunc) {
	...
	for _, tc := range terminalCases {
		t.Run("runtime-"+tc.name, func(t *testing.T) {
			...
			if tc.want == oneshoot.RuntimeTerminalSuccess && runErr != nil {
	...
	t.Run("runtime-cancel", func(t *testing.T) {
		...
		assertRuntimeOutcome(t, result, oneshoot.RuntimeTerminalCancel, root)
	...
	t.Run("runtime-audit", func(t *testing.T) {
		...
		if result.Isolation.SourceWriteAuditStatus != oneshoot.RuntimeAuditViolation || result.Isolation.EvidencePromotion != oneshoot.RuntimePromotionBlocked {
			t.Fatalf("runtime-attributed source write was promotable: %+v", result.Isolation)
		}
		assertRuntimeOutcome(t, result, oneshoot.RuntimeTerminalSuccess, root)
	...
	t.Run("runtime-replay", func(t *testing.T) {
		...
		assertRuntimeOutcome(t, first, oneshoot.RuntimeTerminalSuccess, root)
...
func runtimeSpec(command oneshoot.RuntimeCommand) oneshoot.RuntimeExecutionSpec {
	return oneshoot.RuntimeExecutionSpec{
		Command: command, CommandDigest: oneshoot.CommandDigest(command.Command, command.Args),
		AccessScope:    oneshoot.RuntimeAccessScope{Source: "immutable_snapshot", Network: "disabled", Credentials: "not_available"},
		IsolationScope: oneshoot.RuntimeIsolationScope{Level: "trusted_local", SourceMount: "not_mounted", SourcePermission: "read_only_protocol", WorkingDirectory: "process_private_disposable", WritableLayer: "discarded_after_terminal", RepositoryPathExposed: false},
	}
}

func runtimeConsent(snapshot protocol.Snapshot, spec oneshoot.RuntimeExecutionSpec, consentID, nonce string) oneshoot.RuntimeConsent {
	now := time.Now().UTC()
	return oneshoot.RuntimeConsent{
		SchemaID: oneshoot.RuntimeConsentSchemaID, SchemaVersion: oneshoot.RuntimeSchemaVersion,
...
func runConcurrentLiveEdit(t *testing.T, bin string, spec oneshoot.RuntimeExecutionSpec) RuntimeOutcome {
	t.Helper()
	root := t.TempDir()
	...
	assertRuntimeOutcome(t, result, oneshoot.RuntimeTerminalTimeout, disposableRoot)
	...
	if result.Isolation.SourceWriteAuditStatus != oneshoot.RuntimeAuditClean {
