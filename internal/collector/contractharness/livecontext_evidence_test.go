...
type liveContextExecution struct {
	Package string
	Test    string
}

var liveContextExecutions = map[string]liveContextExecution{
	"VS11-A1": {"codeflow/internal/presenter/flowview", "TestRFLSCR2VS11_A01"},
	"VS11-A2": {"codeflow/internal/presenter/flowview", "TestRFLSCR2VS11_A02"},
	"VS11-A3": {"codeflow/internal/presenter/flowview", "TestRFLSCR2VS11_A03"},
	"VS11-A4": {"codeflow/internal/presenter/flowview", "TestRFLSCR2VS11_A04"},
	"VS11-A5": {"codeflow/internal/presenter/flowview", "TestRFLSCR2VS11_A05"},
	"VS11-A6": {"codeflow/internal/presenter/flowview", "TestRFLSCR2VS11_A06"},
	"VS11-A7": {"codeflow/internal/presenter/flowview", "TestRFLSCR2VS11_A07"},
	"VS11-A8": {"codeflow/internal/presenter/flowview", "TestRFLSCR2VS11_A08"},
}

type liveContextTestEvent struct {
	Action  string
	Package string
	Test    string
	Output  string
...
type liveContextObservedRun struct {
	Events       []liveContextTestEvent
	Records      []evidence.FlowContextRecord
	Binary       string
	BinaryDigest string
	Challenge    string
}
...
func getVS11CompiledBinary(t *testing.T, root string) (string, string) {
	...
	dir, err := os.MkdirTemp("", "livecontext-evidence-bin-*")
	if err != nil {
		t.Fatal(err)
	}
	binary := filepath.Join(dir, "livecontext-evidence.test")
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()

	cmd := exec.CommandContext(ctx, "go", "test", "-c", "-o", binary, "codeflow/internal/presenter/flowview")
	cmd.Dir = root
...
func runLiveContextCriterion(t *testing.T, criterion string) liveContextObservedRun {
	t.Helper()
	execution, ok := liveContextExecutions[criterion]
	if !ok {
	...
	for {
		var event liveContextTestEvent
	...
	for _, log := range logs {
		for _, line := range strings.Split(log, "\n") {
			idx := strings.Index(line, evidence.FlowContextEvidencePrefix)
			if idx < 0 {
				continue
			}
			raw := line[idx+len(evidence.FlowContextEvidencePrefix):]
			var record evidence.FlowContextRecord
...
func TestRFLSCR2VS11_EvidenceRegistry(t *testing.T) {
	...
	if len(liveContextExecutions) != 8 {
		t.Fatalf("expected 8 VS-11 executable registrations, got %d", len(liveContextExecutions))
	}

	allRecords := make([]evidence.FlowContextRecord, 0, len(harness.VS11EvidenceCriteria))
	...
	for _, criterion := range harness.VS11EvidenceCriteria {
		...
		var run liveContextObservedRun
		passed := t.Run(criterion, func(st *testing.T) {
			run = runLiveContextCriterion(st, criterion)
		})
		if !passed {
...
func TestVS11EvidenceRegistryRejectsFabricatedPass(t *testing.T) {
	record := evidence.FlowContextRecord{
		Criterion:          "VS11-A1",
		Package:            "codeflow/internal/presenter/flowview",
		...
		Artifacts: []evidence.FlowContextArtifact{
	...
	// Criterion mismatch
	if err := harness.ValidateVS11EvidenceRecord("VS11-A2", "TestRFLSCR2VS11_A01", record, "codeflow/internal/presenter/flowview", "flowview.test"); err == nil {
	...
	if err := harness.ValidateVS11EvidenceRecord("VS11-A1", "TestRFLSCR2VS11_A01", badRecord, "codeflow/internal/presenter/flowview", "flowview.test"); err == nil {
...
func TestVS11EvidenceRegistryRejectsDuplicateImmutableRef(t *testing.T) {
	...
	if err := harness.ValidateVS11EvidenceRecord("VS11-A1", "TestRFLSCR2VS11_A01", record, "codeflow/internal/presenter/flowview", "flowview.test"); err == nil {
