// Package compiler contains the shared executable criterion runners for the
// semantic compiler contract. The runners deliberately call the public
...
var implementationTestIDs = map[string]string{
	"VS04-A1":  "codeflow/internal/collector/verification/compiler.TestRFLSCR2VS04_A01",
	"VS04-A2":  "codeflow/internal/collector/verification/compiler.TestRFLSCR2VS04_A02",
	"VS04-A3":  "codeflow/internal/collector/verification/compiler.TestRFLSCR2VS04_A03",
	"VS04-A4":  "codeflow/internal/collector/verification/compiler.TestRFLSCR2VS04_A04",
	"VS04-A5":  "codeflow/internal/collector/verification/compiler.TestRFLSCR2VS04_A05",
	"VS04-A6":  "codeflow/internal/collector/verification/compiler.TestRFLSCR2VS04_A06",
	"VS04-A7":  "codeflow/internal/collector/verification/compiler.TestRFLSCR2VS04_A07",
	"VS04-A8":  "codeflow/internal/collector/verification/compiler.TestRFLSCR2VS04_A08",
	"VS04-A9":  "codeflow/internal/collector/verification/compiler.TestRFLSCR2VS04_A09",
	"VS04-A10": "codeflow/internal/collector/verification/compiler.TestRFLSCR2VS04_A10",
	"VS04-A11": "codeflow/internal/collector/verification/compiler.TestRFLSCR2VS04_A11",
	"VS04-A12": "codeflow/internal/collector/verification/compiler.TestRFLSCR2VS04_A12",
	"VS04-A13": "codeflow/internal/collector/verification/compiler.TestRFLSCR2VS04_A13",
	"VS04-A14": "codeflow/internal/collector/verification/compiler.TestRFLSCR2VS04_A14",
}
...
func NewFixture() (*Fixture, error) {
	...
	input, err := evidence.SnapshotInputFromContent("snapshot-vs04", "basis-vs04", "tree-vs04", "config-vs04", "deps-vs04", 11, files)
	if err != nil {
		return nil, err
	}
	target := &semantic.ResolvedTarget{
...
func fixtureSteps(input evidence.SnapshotInput) ([]slicing.SliceStep, error) {
...
func fixtureResult(input evidence.SnapshotInput, payload *slicing.SlicedPayload) (evidence.Result, error) {
	...
	result := evidence.Result{
		...
		Capability: evidence.CapabilityProfile{Adapter: "go", AdapterVersion: "vs04-adapter/1", AnalyzerRevision: "vs04-analyzer/1", Features: []string{"snapshot_bytes"}},
		Coverage:   evidence.Coverage{IncludedSourceRoots: []string{"."}, ExcludedReasons: []string{}, Measured: true},
		Facts:      []evidence.Fact{}, Diagnostics: []evidence.Diagnostic{}, Payload: payloadBytes,
	}
	request, err := evidence.NewAnalyzerRequest(result.RequestID, result.Operation, input, nil, nil)
	if err != nil {
		return evidence.Result{}, err
	}
...
func evidenceFor(criterion string, fixture *Fixture, refs ...string) Evidence {
	...
	return Evidence{Criterion: criterion, ImplementationTestID: implementationTestIDs[criterion], ImplementationPackage: "codeflow/internal/collector/verification/compiler", SnapshotTreeDigest: fixture.Input.RootTreeID, ObjectRefs: objectRefs}
}
...
func RunA04(t *testing.T) Evidence {
	...
	shiftedInput, err := evidence.SnapshotInputFromContent("snapshot-vs04-shifted", "basis-vs04-shifted", "tree-vs04-shifted", "config-vs04-shifted", "deps-vs04-shifted", fixture.Input.WorkspaceEpoch, shiftedFiles)
	if err != nil {
		t.Fatal(err)
	}
