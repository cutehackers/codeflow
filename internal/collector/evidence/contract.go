// Package evidence defines the core analyzer wire contracts, snapshot input
// envelopes, causal observation closures, capability matrices, and cryptographic
// evidence extraction.
package evidence
...
// Stutter-free aliases conforming to Google Go Style.
type Record = EvidenceRecord
type Anchor = EvidenceAnchor
type Request = AnalyzerRequest
type ReadSet = AnalysisReadSet
type Closure = ObservationClosure

// Extract is a stutter-free alias for ExtractEvidence.
func Extract(snapshot SnapshotInput, anchors []EvidenceAnchor) ([]EvidenceRecord, error) {
	return ExtractEvidence(snapshot, anchors)
}
