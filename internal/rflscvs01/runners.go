// Package rflscvs01 contains the shared executable behavior for the R2 VS-01
// criterion tests. The workspace package tests and the contract evidence
// registry both call these runners so the registry cannot drift into a second,
// private implementation of the acceptance behavior.
package rflscvs01
...
func newEngine(t *testing.T) (*workspace.SnapshotEngine, string) {
	t.Helper()
	root := t.TempDir()
...
func applyEdit(t *testing.T, engine *workspace.SnapshotEngine, path, content string, version int) (*workspace.DocumentRevision, *workspace.WorkspaceSnapshot) {
	t.Helper()
	rev, snap, err := engine.ApplyVersionedEdit(context.Background(), workspace.EditRequest{
		Path: path, Content: []byte(content), DocumentVersion: version, Source: workspace.SourceIDEVersioned,
	})
	if err != nil {
...
func evidence(t *testing.T, criterion string, engine *workspace.SnapshotEngine, refs ...string) Evidence {
	t.Helper()
...
func RunA03(t *testing.T) Evidence {
	...
	original[0] = 'X'
	snap.Entries["immutable.go"] = workspace.SnapshotEntry{}
...
func RunA04(t *testing.T) Evidence {
	...
	tx, err := engine.BeginTransaction(workspace.SourceAgentTransaction)
	if err != nil {
		t.Fatal(err)
	}
	if err := tx.StageEdit(workspace.EditRequest{Path: "a.go", Content: []byte("package a\n"), DocumentVersion: 1}); err != nil {
		t.Fatal(err)
	}
	if err := tx.StageEdit(workspace.EditRequest{Path: "b.go", Content: []byte("package b\n"), DocumentVersion: 1}); err != nil {
		t.Fatal(err)
	}
...
func RunA05(t *testing.T) Evidence {
	...
	for _, path := range []string{"../secret.go", outsidePath, "linked.go"} {
		...
		var policyErr *workspace.PathPolicyError
...
func RunA06(t *testing.T) Evidence {
	...
	now := time.Now().UTC()
	unstable := workspace.WatcherObservation{
	...
	if rev.Source != workspace.SourceWatcherFallback || snap.Entries["observed.go"].ContentID != contentID {
...
func RunA07(t *testing.T) Evidence {
	t.Helper()
	root := t.TempDir()
	engine, err := workspace.NewSnapshotEngine(root, 3)
	if err != nil {
		t.Fatal(err)
	}
...
func RunA08(t *testing.T) Evidence {
	t.Helper()
	root := t.TempDir()
	stateDir := filepath.Join(root, workspace.DirName, "workspace")
	...
	historyEntries, err := os.ReadDir(filepath.Join(root, workspace.DirName, "workspace", "historical", "incompatible"))
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, entry := range historyEntries {
		data, readErr := os.ReadFile(filepath.Join(root, workspace.DirName, "workspace", "historical", "incompatible", entry.Name()))
...
func RunA10(t *testing.T) Evidence {
	...
	if err := engine.WriteSource("source.go", []byte("must not overwrite")); !errors.Is(err, workspace.ErrSourceIntegrityViolation) {
