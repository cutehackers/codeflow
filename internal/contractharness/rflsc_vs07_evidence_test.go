...
import (
	...
	"codeflow/internal/rflscvs07runner"
)

const (
	vs07ImplementationPackage = "codeflow/internal/rflscvs07runner"
	vs07ExecutionPackage      = "codeflow/internal/contractharness"
)

type vs07Evidence struct {
...
func TestRFLSCR2VS07_EvidenceRegistry(t *testing.T) {
	...
	runners := map[string]func(*testing.T) rflscvs07runner.Evidence{
		"VS07-A1": rflscvs07runner.RunA01,
		"VS07-A2": rflscvs07runner.RunA02,
		"VS07-A3": rflscvs07runner.RunA03,
		"VS07-A4": rflscvs07runner.RunA04,
		"VS07-A5": rflscvs07runner.RunA05,
		"VS07-A6": rflscvs07runner.RunA06,
		"VS07-A7": rflscvs07runner.RunA07,
	}
...
func vs07ExecutionRecord(t *testing.T, shared rflscvs07runner.Evidence) vs07Evidence {
	t.Helper()
	return vs07Evidence{
		Criterion: shared.Criterion, ImplementationTestID: shared.ImplementationTestID,
		ImplementationPackage: shared.ImplementationPackage, ExecutionPackage: vs07ExecutionPackage,
...
func validateVS07EvidenceRecord(criterion, executionID string, record vs07Evidence, expected map[string]string) error {
	if record.Criterion != criterion {
		return fmt.Errorf("criterion identity mismatch: got %q want %q", record.Criterion, criterion)
	}
	if record.ImplementationTestID != expected[criterion] || record.ImplementationPackage != vs07ImplementationPackage {
		return fmt.Errorf("implementation identity mismatch: %+v", record)
	}
	if record.ExecutionPackage != vs07ExecutionPackage || strings.TrimSpace(record.ExecutionBinary) == "" || record.ExecutionID != executionID {
...
func validateVS07CompletedEvidenceRecord(criterion, executionID string, record vs07Evidence, expected map[string]string) error {
...
func validateVS07RegistryRecords(records []vs07Evidence, expectedCriteria []string) error {
...
func TestVS07EvidenceRegistryRejectsMissingDuplicateNotRunAndFabricatedPass(t *testing.T) {
	...
	expectedImplementation := expectedVS07ImplementationIDs()
	valid := func(criterion, executionID string) vs07Evidence {
		return vs07Evidence{
			Criterion: criterion, ImplementationTestID: expectedImplementation[criterion], ImplementationPackage: vs07ImplementationPackage,
			ExecutionPackage: vs07ExecutionPackage, ExecutionBinary: "contractharness.test", ExecutionID: executionID,
	...
	cases := []struct {
		name    string
		records []vs07Evidence
	}{
		{name: "missing", records: []vs07Evidence{valid("VS07-A1", "run-a1")}},
		{name: "duplicate", records: []vs07Evidence{valid("VS07-A1", "run-a1"), valid("VS07-A1", "run-a1b")}},
		{name: "not-run", records: []vs07Evidence{{Criterion: "VS07-A1"}}},
		{name: "zero-match", records: nil},
		{name: "fabricated-pass", records: []vs07Evidence{func() vs07Evidence {
...
func TestVS07EvidenceRegistryRejectsDuplicateImmutableRef(t *testing.T) {
	record := vs07Evidence{
		Criterion: "VS07-A1", ImplementationTestID: expectedVS07ImplementationIDs()["VS07-A1"], ImplementationPackage: vs07ImplementationPackage,
		ExecutionPackage: vs07ExecutionPackage, ExecutionBinary: "contractharness.test", ExecutionID: "run-a1", Result: "pass", ExecutionCompleted: true, runnerObserved: true,
		SnapshotID: "snapshot-vs07", SnapshotTreeDigest: "tree-vs07", ComputedBasisID: "basis-vs07", GenerationID: "generation-vs07",
