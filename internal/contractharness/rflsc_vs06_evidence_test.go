...
import (
	...
	"codeflow/internal/rflscvs06runner"
	"codeflow/internal/workspace"
	"codeflow/schemas"
)

const (
	vs06ImplementationPackage = "codeflow/internal/rflscvs06runner"
	vs06ExecutionPackage      = "codeflow/internal/contractharness"
)

type vs06Evidence struct {
	...
	RuntimeOutcomes          []rflscvs06runner.RuntimeOutcome `json:"runtimeOutcomes"`
...
func TestRFLSCR2VS06_EvidenceRegistry(t *testing.T) {
	...
	runners := map[string]func(*testing.T) rflscvs06runner.Evidence{
		"VS06-A1": rflscvs06runner.RunA01,
		"VS06-A2": rflscvs06runner.RunA02,
		"VS06-A3": rflscvs06runner.RunA03,
		"VS06-A4": rflscvs06runner.RunA04,
		"VS06-A5": rflscvs06runner.RunA05,
		"VS06-A6": rflscvs06runner.RunA06,
		"VS06-A7": rflscvs06runner.RunA07,
		"VS06-A8": rflscvs06runner.RunA08,
		"VS06-A9": rflscvs06runner.RunA09,
	}
...
func vs06ExecutionRecord(t *testing.T, shared rflscvs06runner.Evidence) vs06Evidence {
	t.Helper()
	...
	return vs06Evidence{
		...
		ExecutionPackage:         vs06ExecutionPackage,
		...
		RuntimeOutcomes:          append([]rflscvs06runner.RuntimeOutcome(nil), shared.RuntimeOutcomes...),
		ObjectRefs:               append([]string(nil), shared.ObjectRefs...),
	}
}

func validateVS06EvidenceRecord(criterion, executionID string, record vs06Evidence, expected map[string]string) error {
	if record.Criterion != criterion {
		return fmt.Errorf("criterion identity mismatch: got %q want %q", record.Criterion, criterion)
	}
	if record.ImplementationTestID != expected[criterion] || record.ImplementationPackage != vs06ImplementationPackage {
		return fmt.Errorf("implementation identity mismatch: %+v", record)
	}
	if record.ExecutionPackage != vs06ExecutionPackage || record.ExecutionBinary == "" || record.ExecutionID != executionID {
...
func validateVS06CompletedEvidenceRecord(criterion, executionID string, record vs06Evidence, expected map[string]string) error {
...
func validateVS06RegistryRecords(records []vs06Evidence, expectedCriteria []string) error {
...
func TestVS06EvidenceRegistryRejectsFabricatedPass(t *testing.T) {
	record := vs06Evidence{
		Criterion: "VS06-A1", ImplementationTestID: "codeflow/internal/rflscvs06runner.TestRFLSCR2VS06_A01", ImplementationPackage: vs06ImplementationPackage,
		ExecutionPackage: vs06ExecutionPackage, ExecutionBinary: "contractharness.test", ExecutionID: "fake-execution", Result: "pass",
...
func TestVS06EvidenceRegistryRejectsIncompleteEvidence(t *testing.T) {
	base := vs06Evidence{
		Criterion: "VS06-A1", ImplementationTestID: "codeflow/internal/rflscvs06runner.TestRFLSCR2VS06_A01", ImplementationPackage: vs06ImplementationPackage,
		ExecutionPackage: vs06ExecutionPackage, ExecutionBinary: "contractharness.test", ExecutionID: "completed-vs06-a1", Result: "pass", ExecutionCompleted: true,
		SnapshotID: "snapshot-vs06", SnapshotTreeDigest: "tree-vs06", InputTreeVerified: true,
		ObjectRefs: []string{"snapshot:snapshot-vs06", "tree:tree-vs06", "basis:basis-vs06", "graph:map-vs06"},
	}
	cases := []struct {
		name   string
		mutate func(*vs06Evidence)
	}{
		{name: "not-run", mutate: func(record *vs06Evidence) {
			record.ExecutionCompleted = false
			record.Result = ""
		}},
		{name: "zero-match", mutate: func(record *vs06Evidence) {
			record.Criterion = "VS06-A0"
		}},
		{name: "duplicate-evidence-ref", mutate: func(record *vs06Evidence) {
			record.ObjectRefs = append(record.ObjectRefs, "graph:map-vs06")
		}},
		{name: "missing-evidence-ref", mutate: func(record *vs06Evidence) {
...
func TestVS06EvidenceRegistryRejectsMissingDuplicateAndZeroMatchRecords(t *testing.T) {
	base := vs06Evidence{Criterion: "VS06-A1"}
	cases := []struct {
		name    string
		records []vs06Evidence
	}{
		{name: "missing", records: []vs06Evidence{base}},
		{name: "duplicate", records: []vs06Evidence{base, base}},
		{name: "zero-match", records: []vs06Evidence{{Criterion: "VS06-A0"}, base}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
