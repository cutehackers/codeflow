...
func TestRFLSCR2VS02_EvidenceRegistry(t *testing.T) {
	...
	runners := map[string]func(*testing.T) rflscvs02.Evidence{
		"VS02-A1": rflscvs02.RunA01, "VS02-A2": rflscvs02.RunA02, "VS02-A3": rflscvs02.RunA03,
		"VS02-A4": rflscvs02.RunA04, "VS02-A5": rflscvs02.RunA05, "VS02-A6": rflscvs02.RunA06,
		"VS02-A7": rflscvs02.RunA07, "VS02-A8": rflscvs02.RunA08, "VS02-A9": rflscvs02.RunA09,
		"VS02-A10": rflscvs02.RunA10, "VS02-A11": rflscvs02.RunA11,
	}
	...
	for _, criterion := range expected {
		...
		var evidence rflscvs02.Evidence
		var record vs02Evidence
		if !t.Run(criterion, func(t *testing.T) {
			evidence = runner(t)
			validateVS02Evidence(t, criterion, evidence)
			record = evidenceRecordVS02(t, evidence)
			...
			record.Result = "pass"
			if err := validateVS02EvidenceRecord(criterion, t.Name(), record); err != nil {
				t.Fatal(err)
			}
		}) {
...
func validateVS02Evidence(t *testing.T, criterion string, evidence rflscvs02.Evidence) {
	t.Helper()
...
func evidenceRecordVS02(t *testing.T, shared rflscvs02.Evidence) vs02Evidence {
	t.Helper()
	return vs02Evidence{
		Criterion: shared.Criterion, ImplementationTestID: shared.ImplementationTestID,
		ImplementationPackage: "codeflow/internal/rflscvs02", ExecutionPackage: "codeflow/internal/contractharness",
...
func validateVS02EvidenceRecord(criterion, executionID string, record vs02Evidence) error {
	...
	if record.ImplementationPackage != "codeflow/internal/rflscvs02" || record.ExecutionPackage != "codeflow/internal/contractharness" || record.ExecutionBinary == "" || record.ExecutionID != executionID {
		return fmt.Errorf("incomplete execution identity: %+v", record)
	}
	return nil
}

func validateVS02EvidenceFields(criterion, gotCriterion, implementationID, result, treeDigest string, audit workspace.SourceWriteAudit, isolation rflscvs02.MountPermissionEvidence, refs []string) error {
...
func TestVS02EvidenceRegistryRejectsFabricatedPass(t *testing.T) {
	record := vs02Evidence{
		Criterion: "VS02-A11", ImplementationTestID: "fabricated", ImplementationPackage: "codeflow/internal/rflscvs02",
		ExecutionPackage: "codeflow/internal/contractharness", ExecutionBinary: "test", ExecutionID: "fake", Result: "pass",
		SnapshotTreeDigest: "tree", RepositoryPathWriteAudit: workspace.SourceWriteAudit{CapturedSnapshotTreeDigest: "tree"},
		MountPermissionEvidence: rflscvs02.MountPermissionEvidence{
	...
	if err := validateVS02EvidenceRecord(record.Criterion, "fake", record); err == nil {
