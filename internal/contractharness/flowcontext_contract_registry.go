...
type flowContextContractDef struct {
	producer string
	consumer string
}

var expectedFlowContextContractDefs = map[string]flowContextContractDef{
...
type flowContextInvalidFixture struct {
	category string
	field    string
}

var expectedFlowContextInvalidFixtures = map[string]map[string]flowContextInvalidFixture{
...
func flowContextRegistryFixtures(entry VS09ContractRegistryEntry) []flowContextFixture {
	return []flowContextFixture{
...
func validateVS09ContractRegistryMetadata(entry VS09ContractRegistryEntry) error {
	...
	definition, ok := expectedFlowContextContractDefs[entry.ID]
	if !ok {
	...
	for _, fixture := range flowContextRegistryFixtures(entry) {
...
func ValidateVS09ContractRegistryEntry(entry VS09ContractRegistryEntry, valid, invalidAuthority, invalidReference, invalidVersion, invalidTransition []byte) error {
	...
	for _, fixture := range []struct {
		...
		expectation := expectedFlowContextInvalidFixtures[entry.ID][fixture.class]
		err := validator(fixture.data)
		if err == nil {
			return fmt.Errorf("%s invalid %s fixture %q unexpectedly passed validation", entry.ID, fixture.class, flowContextFixturePath(entry, fixture.class))
		}
...
func ValidateVS09ContractRegistry() error {
	if len(VS09ContractRegistry) != len(expectedFlowContextContractDefs) {
		return fmt.Errorf("VS-09 contract registry count=%d want=%d", len(VS09ContractRegistry), len(expectedFlowContextContractDefs))
	}
	...
	if len(seenIDs) != len(expectedFlowContextContractDefs) || len(seenFixtures) != len(expectedFlowContextContractDefs)*5 {
		return fmt.Errorf("VS-09 contract registry identity or fixture set is incomplete")
	}
	return nil
}

func flowContextFixturePath(entry VS09ContractRegistryEntry, class string) string {
	for _, fixture := range flowContextRegistryFixtures(entry) {
