...
type vs08ContractDefinition struct {
	producer string
	consumer string
}

var expectedVS08ContractDefinitions = map[string]vs08ContractDefinition{
...
type vs08InvalidFixtureExpectation struct {
	instanceLocation string
	keyword          string
}

// expectedVS08InvalidFixtureExpectations binds each named invalid fixture to
...
var expectedVS08InvalidFixtureExpectations = map[string]map[string]vs08InvalidFixtureExpectation{
...
type vs08Fixture struct {
	class string
	path  string
}

func vs08RegistryFixtures(entry VS08ContractRegistryEntry) []vs08Fixture {
	return []vs08Fixture{
...
func validateVS08ContractRegistryMetadata(entry VS08ContractRegistryEntry) error {
	...
	definition, ok := expectedVS08ContractDefinitions[entry.ID]
	if !ok {
	...
	seenFixtures := make(map[string]string, len(vs08RegistryFixtures(entry)))
	for _, fixture := range vs08RegistryFixtures(entry) {
...
func ValidateVS08ContractRegistryEntry(entry VS08ContractRegistryEntry, valid, invalidAuthority, invalidReference, invalidBound []byte) error {
	...
	expectations, ok := expectedVS08InvalidFixtureExpectations[entry.ID]
	if !ok {
	...
	for _, fixture := range fixtures[1:] {
		...
		if err := validateVS08InvalidFixture(entry.SchemaID, fixture.data, expectation); err != nil {
			return fmt.Errorf("%s %s fixture %q: %w", entry.ID, fixture.class, vs08FixturePath(entry, fixture.class), err)
		}
	}
	return nil
}

type vs08ValidationEvidence struct {
	instanceLocation string
	keyword          string
}

func validateVS08InvalidFixture(schemaID string, data []byte, expectation vs08InvalidFixtureExpectation) error {
	...
	evidence := make([]vs08ValidationEvidence, 0, 1)
	...
	actual := evidence[0]
	if actual != (vs08ValidationEvidence(expectation)) {
...
func collectVS08ValidationEvidence(err *jsonschema.ValidationError, evidence *[]vs08ValidationEvidence) {
	if len(err.Causes) == 0 {
		...
		*evidence = append(*evidence, vs08ValidationEvidence{
			instanceLocation: vs08JSONPointer(err.InstanceLocation),
			keyword:          keyword,
		})
		return
	}
...
func vs08JSONPointer(parts []string) string {
	if len(parts) == 0 {
		return ""
	}
...
func vs08FixturePath(entry VS08ContractRegistryEntry, class string) string {
	for _, fixture := range vs08RegistryFixtures(entry) {
...
func ValidateVS08ContractRegistry() error {
	if len(VS08ContractRegistry) != len(expectedVS08ContractDefinitions) {
		return fmt.Errorf("VS-08 contract registry count=%d want=%d", len(VS08ContractRegistry), len(expectedVS08ContractDefinitions))
	}
	...
	if len(seenIDs) != len(expectedVS08ContractDefinitions) {
		return fmt.Errorf("VS-08 contract registry ID set has %d entries want %d", len(seenIDs), len(expectedVS08ContractDefinitions))
	}
	return nil
}
