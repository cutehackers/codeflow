...
import (
	...
	"codeflow/internal/rflscvs05"
	"codeflow/schemas"
)

const (
	vs05ImplementationPackage = "codeflow/internal/rflscvs05"
	vs05ExecutionPackage      = "codeflow/internal/contractharness"
)

type vs05Evidence struct {
...
func TestRFLSCR2VS05_EvidenceRegistry(t *testing.T) {
	...
	runners := map[string]func(*testing.T) rflscvs05.Evidence{
		"VS05-A1": rflscvs05.RunA01,
		"VS05-A2": rflscvs05.RunA02,
		"VS05-A3": rflscvs05.RunA03,
		"VS05-A4": rflscvs05.RunA04,
		"VS05-A5": rflscvs05.RunA05,
		"VS05-A6": rflscvs05.RunA06,
		"VS05-A7": rflscvs05.RunA07,
	}
	expectedImplementation := expectedVS05ImplementationIDs()
	records := make([]vs05Evidence, 0, len(expectedCriteria))
	for _, criterion := range expectedCriteria {
		runner := runners[criterion]
		if runner == nil {
			t.Fatalf("missing VS-05 evidence runner for %s", criterion)
		}
		var record vs05Evidence
...
func TestVS05EvidenceRegistryRejectsMissingDuplicateNotRunAndFabricatedPass(t *testing.T) {
	...
	expectedImplementation := expectedVS05ImplementationIDs()
	valid := func(criterion, executionID string) vs05Evidence {
		return vs05Evidence{
			Criterion: criterion, ImplementationTestID: expectedImplementation[criterion],
			ImplementationPackage: vs05ImplementationPackage, ExecutionPackage: vs05ExecutionPackage,
	...
	cases := []struct {
		name    string
		records []vs05Evidence
	}{
		{name: "missing", records: []vs05Evidence{valid("VS05-A1", "run-a1")}},
		{name: "duplicate", records: []vs05Evidence{valid("VS05-A1", "run-a1"), valid("VS05-A1", "run-a1b")}},
		{name: "not-run", records: []vs05Evidence{{Criterion: "VS05-A1"}}},
		{name: "zero-match", records: nil},
		{name: "fabricated-pass", records: []vs05Evidence{valid("VS05-A1", "fake-execution")}},
	}
...
func expectedVS05ImplementationIDs() map[string]string {
	return map[string]string{
		"VS05-A1": "codeflow/internal/rflscvs05.TestRFLSCR2VS05_A01",
		"VS05-A2": "codeflow/internal/rflscvs05.TestRFLSCR2VS05_A02",
		"VS05-A3": "codeflow/internal/rflscvs05.TestRFLSCR2VS05_A03",
		"VS05-A4": "codeflow/internal/rflscvs05.TestRFLSCR2VS05_A04",
		"VS05-A5": "codeflow/internal/rflscvs05.TestRFLSCR2VS05_A05",
		"VS05-A6": "codeflow/internal/rflscvs05.TestRFLSCR2VS05_A06",
		"VS05-A7": "codeflow/internal/rflscvs05.TestRFLSCR2VS05_A07",
	}
}

func vs05ExecutionRecord(t *testing.T, shared rflscvs05.Evidence) vs05Evidence {
	t.Helper()
	return vs05Evidence{
		...
		ExecutionPackage:      vs05ExecutionPackage,
...
func validateVS05EvidenceRecord(criterion, executionID string, record vs05Evidence, expected map[string]string) error {
	if record.Criterion != criterion {
		return fmt.Errorf("criterion identity mismatch: got %q want %q", record.Criterion, criterion)
	}
	if expected[criterion] == "" || record.ImplementationTestID != expected[criterion] || record.ImplementationPackage != vs05ImplementationPackage {
		return fmt.Errorf("implementation identity mismatch: %+v", record)
	}
	if record.ExecutionPackage != vs05ExecutionPackage || record.ExecutionBinary == "" || record.ExecutionID != executionID {
...
func validateCompletedVS05Records(records []vs05Evidence, expectedCriteria []string, expected map[string]string) error {
