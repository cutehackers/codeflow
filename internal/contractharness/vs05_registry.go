...
func vs05String(doc map[string]any, key string) string {
	v, _ := doc[key].(string)
	return v
}

func vs05Number(doc map[string]any, key string) (float64, bool) {
	v, ok := doc[key].(float64)
	return v, ok
}

func vs05Object(doc map[string]any, key string) map[string]any {
	v, _ := doc[key].(map[string]any)
	return v
}

func vs05Array(doc map[string]any, key string) []any {
	v, _ := doc[key].([]any)
	return v
}
...
func ValidateImpactQueryV2(data []byte) error {
	...
	if basis := vs05String(doc, "computedBasisId"); basis == "" {
		return fmt.Errorf("impact query computedBasisId is required")
	}
	if generation := vs05String(doc, "generationId"); generation == "" {
		return fmt.Errorf("impact query generationId is required")
	}
	depth, depthOK := vs05Number(doc, "maxDepth")
	maxNodes, nodesOK := vs05Number(doc, "maxNodes")
	...
	if len(vs05Array(doc, "relationKinds")) == 0 {
		return fmt.Errorf("impact query relationKinds must not be empty")
	}
	if vs05String(doc, "freshness") == "current" {
...
func ValidateChangeImpactGraphV2(data []byte) error {
	...
	basis := vs05String(doc, "computedBasisId")
	generation := vs05String(doc, "generationId")
	snapshot := vs05String(doc, "validatedAgainstSnapshotId")
	...
	for _, raw := range vs05Array(doc, "evidence") {
		...
		if vs05String(ev, "computedBasisId") != basis || vs05String(ev, "snapshotId") != snapshot {
			return fmt.Errorf("impact evidence %q is not bound to the graph basis/snapshot", id)
		}
		if vs05String(ev, "validationStatus") != "verified" {
			return fmt.Errorf("impact evidence %q is not verified", id)
		}
		anchor := vs05Object(ev, "anchor")
		if anchor == nil || vs05String(anchor, "repoRelativePath") == "" {
	...
	validateSet := func(name string, set map[string]any, indirect bool) error {
		...
		if indirect {
			depth, depthOK := vs05Number(set, "maxDepth")
			nodes, nodesOK := vs05Number(set, "maxNodes")
			explored, exploredOK := vs05Number(set, "exploredDepth")
			count, countOK := vs05Number(set, "totalNodeCount")
		...
		if err := validateVS05ClaimArray(vs05Array(set, "callers"), "caller", evidenceByID, target, indirect); err != nil {
			return err
		}
		if err := validateVS05ClaimArray(vs05Array(set, "stateMutations"), "stateMutation", evidenceByID, target, indirect); err != nil {
			return err
		}
		if err := validateVS05ClaimArray(vs05Array(set, "externalEffects"), "externalEffect", evidenceByID, target, indirect); err != nil {
			return err
		}
		if err := validateVS05ClaimArray(vs05Array(set, "relatedFlows"), "relatedFlow", evidenceByID, target, indirect); err != nil {
			return err
		}
		if err := validateVS05ClaimArray(vs05Array(set, "tests"), "test", evidenceByID, target, indirect); err != nil {
			return err
		}
		return nil
	}
	if err := validateSet("directImpact", vs05Object(doc, "directImpact"), false); err != nil {
		return err
	}
	if err := validateSet("indirectImpact", vs05Object(doc, "indirectImpact"), true); err != nil {
		return err
	}

	frontiers := vs05Array(doc, "frontiers")
	unknown, unknownOK := vs05Number(doc, "unknownCount")
	...
	for _, raw := range frontiers {
		...
		if vs05String(frontier, "schemaId") != ImpactFrontierV1SchemaID || vs05NumberValue(frontier, "schemaVersion") != 1 {
		...
		for _, rawRef := range vs05Array(frontier, "evidenceRefs") {
			ref, _ := rawRef.(string)
	...
	indirect := vs05Object(doc, "indirectImpact")
	...
	unresolved := vs05Array(doc, "unresolvedBoundaries")
	...
	for i, raw := range unresolved {
		...
		if !ok || !frontierOK || vs05String(boundary, "boundaryType") != vs05String(frontier, "boundaryType") || vs05String(boundary, "target") != vs05String(frontier, "target") || vs05String(boundary, "description") != vs05String(frontier, "reason") {
...
func vs05NumberValue(doc map[string]any, key string) float64 {
	v, _ := vs05Number(doc, key)
	return v
}
...
func validateVS05ClaimArray(values []any, kind string, evidenceByID map[string]map[string]any, target map[string]any, indirect bool) error {
	for _, raw := range values {
		...
		depth, depthOK := vs05Number(claim, "depth")
		path := vs05Array(claim, "path")
		...
		terminal, err := vs05ClaimTerminalSymbol(kind, claim)
		if err != nil {
			return err
		}
		...
		if kind == "caller" {
			caller := vs05String(claim, "symbolPath")
			if caller == "" {
		...
		for _, rawRef := range vs05Array(claim, "evidenceRefs") {
			...
			anchor := vs05Object(evidence, "anchor")
			if anchor == nil || vs05String(anchor, "enclosingSymbolPath") != terminal {
				return fmt.Errorf("impact %s claim Evidence %q is not anchored to terminal symbol %q", kind, ref, terminal)
			}
			if kind == "caller" && vs05String(anchor, "repoRelativePath") != vs05String(claim, "filePath") {
				return fmt.Errorf("impact caller claim Evidence %q is not anchored to filePath %q", ref, vs05String(claim, "filePath"))
			}
		}
	}
	return nil
...
func vs05ClaimTerminalSymbol(kind string, claim map[string]any) (string, error) {
	key := "terminalSymbolPath"
	if kind == "caller" {
		key = "symbolPath"
	}
	terminal := vs05String(claim, key)
	if terminal == "" {
