...
type vs09Execution struct {
	Package, Test string
	Observations  int
	Overlay       bool
}

var vs09Executions = map[string]vs09Execution{
...
type vs09TestEvent struct{ Action, Package, Test, Output string }
type vs09ObservedRun struct {
	Events                          []vs09TestEvent
	Records                         []rflscvs09evidence.Record
...
func TestRFLSCR2VS09_A01(t *testing.T) { runVS09Criterion(t, "VS09-A1") }
func TestRFLSCR2VS09_A02(t *testing.T) { runVS09Criterion(t, "VS09-A2") }
func TestRFLSCR2VS09_A03(t *testing.T) { runVS09Criterion(t, "VS09-A3") }
func TestRFLSCR2VS09_A04(t *testing.T) { runVS09Criterion(t, "VS09-A4") }
func TestRFLSCR2VS09_A05(t *testing.T) { runVS09Criterion(t, "VS09-A5") }
func TestRFLSCR2VS09_A06(t *testing.T) { runVS09Criterion(t, "VS09-A6") }
func TestRFLSCR2VS09_A07(t *testing.T) { runVS09Criterion(t, "VS09-A7") }
func TestRFLSCR2VS09_A08(t *testing.T) { runVS09Criterion(t, "VS09-A8") }
func TestRFLSCR2VS09_A09(t *testing.T) { runVS09Criterion(t, "VS09-A9") }
func TestRFLSCR2VS09_A10(t *testing.T) { runVS09Criterion(t, "VS09-A10") }
func TestRFLSCR2VS09_A11(t *testing.T) { runVS09Criterion(t, "VS09-A11") }
func TestRFLSCR2VS09_A12(t *testing.T) { runVS09Criterion(t, "VS09-A12") }
func TestRFLSCR2VS09_A13(t *testing.T) { runVS09Criterion(t, "VS09-A13") }

func TestRFLSCR2VS09_EvidenceRegistry(t *testing.T) {
	if harness.VS09EvidenceRegistryID != "rflsc-r2-vs-09" || len(vs09Executions) != 13 {
		t.Fatal("missing VS09 executable registrations")
	}
	for i := 1; i <= 13; i++ {
		criterion := fmt.Sprintf("VS09-A%d", i)
		if !t.Run(criterion, func(t *testing.T) { runVS09Criterion(t, criterion) }) {
			t.Fatalf("criterion %s did not complete", criterion)
		}
	}
}

func runVS09Criterion(t *testing.T, criterion string) vs09ObservedRun {
	t.Helper()
	execution, ok := vs09Executions[criterion]
	if !ok {
	...
	defer cancel()
	binary := filepath.Join(t.TempDir(), "vs09-evidence.test")
	args := []string{"test", "-c", "-o", binary, execution.Package}
	if execution.Overlay {
		args = append(args, "-overlay", rflscvs09evidence.FaultOverlay(t, root))
	}
	...
	run := vs09ObservedRun{Binary: filepath.Base(binary), BinaryDigest: hex.EncodeToString(digest[:]), Challenge: hex.EncodeToString(challengeBytes)}
	...
	for _, log := range logs {
		for _, line := range strings.Split(log, "\n") {
			offset := strings.Index(line, rflscvs09evidence.Prefix)
			if offset < 0 {
				continue
			}
			var record rflscvs09evidence.Record
			if err := json.Unmarshal([]byte(line[offset+len(rflscvs09evidence.Prefix):]), &record); err != nil {
...
func TestVS09ApprovalEvidenceRejectsUnobservedAndTamperedRecords(t *testing.T) {
	...
	good := runVS09Criterion(t, "VS09-A1")
	for _, tc := range []struct {
		name   string
		mutate func(*vs09ObservedRun)
	}{
		{"no-test-run", func(r *vs09ObservedRun) { r.Events = nil }},
		{"missing-record", func(r *vs09ObservedRun) { r.Records = r.Records[1:] }},
		{"duplicate-record", func(r *vs09ObservedRun) { r.Records[1] = r.Records[0] }},
		{"skipped", func(r *vs09ObservedRun) {
			r.Events = append(r.Events, vs09TestEvent{Action: "skip", Package: vs09Executions["VS09-A1"].Package, Test: vs09Executions["VS09-A1"].Test})
		}},
		{"failed", func(r *vs09ObservedRun) {
			r.Events = append(r.Events, vs09TestEvent{Action: "fail", Package: vs09Executions["VS09-A1"].Package, Test: vs09Executions["VS09-A1"].Test})
		}},
		{"zero-match", func(r *vs09ObservedRun) {
			r.Events = []vs09TestEvent{{Action: "pass", Package: vs09Executions["VS09-A1"].Package}}
		}},
		{"criterion", func(r *vs09ObservedRun) { r.Records[0].Criterion = "VS09-A13" }},
		{"challenge", func(r *vs09ObservedRun) { r.Records[0].Challenge = "fabricated" }},
		{"package", func(r *vs09ObservedRun) { r.Records[0].Package = "fabricated/package" }},
		{"binary", func(r *vs09ObservedRun) { r.Records[0].Binary = "fabricated.test" }},
		{"binary-digest", func(r *vs09ObservedRun) { r.Records[0].BinaryDigest = strings.Repeat("0", 64) }},
		{"test-id", func(r *vs09ObservedRun) { r.Records[0].ExecutionID = "TestFabricated" }},
		{"artifact-ref", func(r *vs09ObservedRun) { r.Records[0].Artifacts[0].Ref = "sha256:" + strings.Repeat("0", 64) }},
		{"artifact-bytes", func(r *vs09ObservedRun) { r.Records[0].Artifacts[0].Bytes = []byte("fabricated") }},
		{"artifact-rehashed", func(r *vs09ObservedRun) {
			artifact := &r.Records[0].Artifacts[0]
			artifact.Bytes = []byte("fabricated")
			digest := sha256.Sum256(artifact.Bytes)
			artifact.Ref = "sha256:" + hex.EncodeToString(digest[:])
		}},
		{"missing-artifacts", func(r *vs09ObservedRun) {
			for i := range r.Records {
				r.Records[i].Artifacts = nil
			}
		}},
		{"duplicate-artifact", func(r *vs09ObservedRun) {
			r.Records[0].Artifacts = append(r.Records[0].Artifacts, r.Records[0].Artifacts[0])
		}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			raw, _ := json.Marshal(good)
			var changed vs09ObservedRun
			if err := json.Unmarshal(raw, &changed); err != nil {
				t.Fatal(err)
			}
			tc.mutate(&changed)
			if _, err := validateVS09ObservedRun("VS09-A1", vs09Executions["VS09-A1"], changed); err == nil {
