...
type Evidence struct {
	...
	MountPermissionEvidence  MountPermissionEvidence
	ObjectRefs               []string
}

var implementationTestIDs = map[string]string{
	"VS02-A1":  "codeflow/internal/rflscvs02.TestRFLSCR2VS02_A01",
	"VS02-A2":  "codeflow/internal/rflscvs02.TestRFLSCR2VS02_A02",
	"VS02-A3":  "codeflow/internal/rflscvs02.TestRFLSCR2VS02_A03",
	"VS02-A4":  "codeflow/internal/rflscvs02.TestRFLSCR2VS02_A04",
	"VS02-A5":  "codeflow/internal/rflscvs02.TestRFLSCR2VS02_A05",
	"VS02-A6":  "codeflow/internal/rflscvs02.TestRFLSCR2VS02_A06",
	"VS02-A7":  "codeflow/internal/rflscvs02.TestRFLSCR2VS02_A07",
	"VS02-A8":  "codeflow/internal/rflscvs02.TestRFLSCR2VS02_A08",
	"VS02-A9":  "codeflow/internal/rflscvs02.TestRFLSCR2VS02_A09",
	"VS02-A10": "codeflow/internal/rflscvs02.TestRFLSCR2VS02_A10",
	"VS02-A11": "codeflow/internal/rflscvs02.TestRFLSCR2VS02_A11",
}
...
func runnerInput(t *testing.T) (SnapshotInput, workspace.SourceWriteAudit) {
	t.Helper()
...
func runnerEvidence(t *testing.T, criterion string, input SnapshotInput) Evidence {
	t.Helper()
	return Evidence{
		Criterion:                criterion,
...
func runnerRequest(t *testing.T, input SnapshotInput, required ...string) AnalyzerRequest {
	t.Helper()
	request, err := NewAnalyzerRequest("runner-"+t.Name(), "slice", input, []string{"lib/feature.go"}, required)
	if err != nil {
		t.Fatal(err)
	}
	return request
...
func runnerReadDocuments(input SnapshotInput) []ReadDocument {
	documents := make([]ReadDocument, 0, len(input.Documents))
	for _, doc := range input.Documents {
		documents = append(documents, ReadDocument{
...
func measuredObservation(kind string) Observation {
	return Observation{Kind: kind, Path: "lib/feature.go", ValueHash: "measured-" + kind, Measured: true}
}

func runnerValidResult(t *testing.T, request AnalyzerRequest) Result {
	t.Helper()
	readSet := AnalysisReadSet{
		SchemaID: ReadSetSchemaID, SchemaVersion: SchemaVersion, ReadSetID: "runner-read-set",
	...
	closure := ObservationClosure{
		SchemaID: ClosureSchemaID, SchemaVersion: SchemaVersion, ClosureID: "runner-closure",
		...
		NegativeObservations: []Observation{}, MembershipObservations: []Observation{}, DependencyFrontiers: []Observation{},
	...
	return Result{
		SchemaID: AnalyzerResultSchemaID, SchemaVersion: SchemaVersion, RequestID: request.RequestID,
		...
		Capability: CapabilityProfile{Adapter: "runner", AdapterVersion: "2", AnalyzerRevision: "runner-analyzer/2", Features: []string{"snapshot_bytes"}},
...
func RunA02(t *testing.T) Evidence {
	t.Helper()
	input, _ := runnerInput(t)
	// Exercise the real adapter source abstractions. A constructed Result cannot
...
func RunA05(t *testing.T) Evidence {
	...
	if _, err := ExtractEvidence(input, []EvidenceAnchor{{EvidenceID: "runner-evidence", Path: doc.Path, RevisionID: doc.RevisionID, StartByte: 0, EndByte: len(doc.Bytes)}}); err != nil {
...
func RunA06(t *testing.T) Evidence {
	t.Helper()
	input, _ := runnerInput(t)
	_, err := ExtractEvidence(input, []EvidenceAnchor{{EvidenceID: "runner-invalid", Path: "lib/feature.go", RevisionID: "wrong", StartByte: 0, EndByte: 1}})
...
func RunA07(t *testing.T) Evidence {
	t.Helper()
	input, _ := runnerInput(t)
	runnerGoTest(t, "./internal/secret", "TestRedact(MalformedJSONQuotedKeyBeforeClip|RedactAndClipNestedDiagnosticsBeforeBound|RedactAndClipPointerStructuredDiagnostics)")
	runnerGoTest(t, "./internal/protocol", "TestProtocol(ReadLoopRedactsMalformedJSONBeforeDiagnosticClip|StderrTailRedactsAcrossChunksAndTailCutoff|StderrTailDoesNotFinalizeLookbehindBetweenChunks|StderrEOFFinalizesSafeLookbehindButDropsSecret|StderrRedactsLongCredentialKeySplitBeyondLookbehind)|TestRPCErrorStructuredDataRedactsBeforeDecodeAndClip")
...
func RunA08(t *testing.T) Evidence {
	t.Helper()
	input, _ := runnerInput(t)
	runnerGoTest(t, "./internal/protocol", "TestMockAdapterConformance/(Timeout|CancelMidCall|CrashRestartOnce)")
	runnerGoTest(t, "./internal/protocol", "^TestSourceReadOnlyAdapterLifecycle$")
...
func RunA09(t *testing.T) Evidence {
	t.Helper()
	input, _ := runnerInput(t)
	runnerGoTest(t, "./internal/protocol", "^TestOutbound(FrameBoundExactAndOversize|NotificationUsesNegotiatedBound)$")
...
func RunA10(t *testing.T) Evidence {
	t.Helper()
	input, _ := runnerInput(t)
	runnerGoTest(t, "./internal/protocol", "^TestSupportedAdapterInitializeCapabilityMeasurements$")
...
func RunA11(t *testing.T) Evidence {
	...
	runnerGoTestEnv(t, "./internal/protocol", "^(TestAdapterProcessIsolationAndCleanup|TestConnPublishesMountPermissionEvidenceAfterCleanup|TestSourceReadOnlyAdapterLifecycle)$", map[string]string{
	...
	var artifact IsolationLifecycleEvidence
